 ...
 * @author abbas
 */
public class MyFilterAction implements Filter {
    ...
    public void init(FilterConfig config) {
        ...
        MyLogAction.log(fc.getFilterName() + " initialized.");
    }
    ...
    public void doFilter(ServletRequest req, ServletResponse resp,
        ...
        MyLogAction.log("in MyFilter.doFilter with " +
                request.getPathInfo()); // + " @ " + Util1.getStackTrace());
        MyLogAction.log("requestURI:" + request.getRequestURI());
        if (RuntimeSettingsAction.IS_IN_DEBUG_MODE) {
            displayRequestParameters(request);
            MyLogAction.log("will NOT compress the response");
            chain.doFilter(req, resp);
            MyLogAction.log("response:" + resp);
            return;
        }
        chain.doFilter(req, resp);
    }
