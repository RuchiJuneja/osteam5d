...
public class ReporterAction {
	...
	public ReporterAction(String reportType) {
		messageCount = 0;
		reportSuffix = 1;
	...
	public void print(String message, boolean replaceCRLF) {
		if (messageCount == 0) {
			AU1Action.reportMessage(writer, // RuntimeSettings.VERSION_ID +
					"  Report started at: " + AU1Action.getLogCurrDateTime(), 2);
		}
		messageCount++;
		String prefix = " ";
		if (this.reportType.equals("log_report")) {
			prefix = "@: " + AU1Action.getLogCurrDateTime().substring(12) + " ";
		...
		AU1Action.reportMessage(writer, reportMessage, 1);
	...
	public static ReporterAction get(String reportType) {
		ReporterAction result = (ReporterAction) reporters.get(reportType);
		if (result == null) {
			result = new ReporterAction(reportType);
