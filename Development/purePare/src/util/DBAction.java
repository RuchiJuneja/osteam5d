...
import util.AU1Action;
import util.MyLogAction;
import util.RuntimeSettingsAction;
...
public class DBAction {
	...
	public static void main(String[] args) {
		...
		db.runScript(RuntimeSettingsAction.scriptSql);
	}
	...
	public void createDatabase(String databaseName) {
		try {
			...
			Connection con = DriverManager.getConnection(url,
					RuntimeSettingsAction.dbUserID, 
					RuntimeSettingsAction.dbPassword);
	...
	public static DataSource getDataSource() {
		...
		try {
			...
			if (ctx == null) {
				MyLogAction.log_and_print("context is null! in loginaction");
				data = null;
				return null;
			...
			if (data == null) {
				MyLogAction.log_and_print("did NOT get datasource!");
			}
			// else {
			...
			MyLogAction.log_and_print("exception in getDatasource()");
		}
		return data;
	}
	...
	public static int getCount(Connection con, String tableName,
		...
		try {
			...
			MyLogAction.myIO("catch in Taxi.findCount() line # 60");
		}
		close(rs);
		return result;
	}
	...
	private static ArrayList<String> getColumnNames(ResultSet rs) {
		...
		try {
			...
			MyLogAction.myCatch("/java", 188, e);
		}
		return result;
	...
	private DBAction(String password, String dbname) {
		try {
			Class.forName("com.mysql.jdbc.Driver");
			...
			MyLogAction.myCatch("/java", 100, se);
			se.printStackTrace();
		} catch (ClassNotFoundException c) {
			MyLogAction.myCatch("/java", 103, c);
			c.printStackTrace();
		}

	}

	public static DBAction getInstance(String password, String dbname) {
		return new DBAction(password, dbname);
	}
	...
	public static int updateElseInsertTMS(String updateSQL, String insertSQL) {
		...
		if (!isOK) {
			MyLogAction.myIO("in updateElseInsertTMS, incorrect parameters:"
	...
	public static int update(String sql) {
		...
		try {
			...
			MyLogAction.myCatch("/java", 143, ex);
		} finally {
			close(connection);
		}
		return rowsUpdated;
	...
	static void text2Table(String fileName) {
		...
		linesFromFile = AU1Action.textFile2Lines(fileName);
		for (int i = 0; i < linesFromFile.length; i++) {
			if (linesFromFile[i].trim().startsWith("Running query :")) {
				...
				try {
					...
					MyLogAction.myCatch("java", 285, e);
				} finally {
					close(rs);
					close(con);
				}
	...
	public static ResultSet readFromDB(String query, Connection connection) {
		...
		Statement stmt = null;
		try {
			MyLogAction.log("in readFromDB with query:\n" + query);
			if ((query.indexOf(" where ") >= 0)
					|| (query.indexOf("count ") >= 0)) {
				// ok
			} else {
				MyLogAction.myIO("@@@ please check as query without a where clause!");
			}
			...
			MyLogAction.myIO("readFromDB sql:" + query);
		} catch (SQLException se) {
			MyLogAction.myCatch("/java", 66, se);
		}
		return result;
	}
	...
	public static int update(Connection connection, String sql) {
		...
		try {
			...
			MyLogAction.myIO("updating " + rows + " rows for sql:" + sql + ":");
		} catch (SQLException ex) {
			MyLogAction.myCatch("/update() while running sql:" + sql + ":", 79, ex);
		} finally {
			close(statement);
		}
		return rows;
	...
	public static void close(Connection connection) {
		...
		try {
			if (connection.isClosed()) {
				...
				try {
					connection.close();
					MyLogAction.log("closing a conn; total conn:" + activeConnections);
					connection = null;
				} catch (SQLException ex) {
					MyLogAction.myCatch("/java", 106, ex);
				}
			}
		} catch (SQLException ex) {
			MyLogAction.myCatch("/java", 110, ex);
		}
	}
		...
		try {
			...
			if (isSeekingConnectionFirstTime) {
				...
				try {
					...
					MyLogAction.myCatch("/java", 162, ex);
				}
			}
			String connectionUrl = "jdbc:mysql://" + RuntimeSettingsAction.SERVER_IP
					+ ":" + RuntimeSettingsAction.portNo + "/"
					+ RuntimeSettingsAction.databaseName
					+ "?zeroDateTimeBehavior=convertToNull";
			con = DriverManager.getConnection(connectionUrl,
					RuntimeSettingsAction.dbUserID, RuntimeSettingsAction.dbPassword);
		} catch (SQLException ex) {
			MyLogAction.myCatch("/java", 217, ex);
		}
		activeConnections++;
		MyLogAction.log("in DBDataStandAlone added new tms conn; total connections:"
				+ activeConnections);
		return con;
	}
	...
	public static void close(Statement statement) {
		if (statement != null) {
			try {
				...
				MyLogAction.myCatch("/java", 202, ex);
			}
		}
	}
	...
	public static void close(ResultSet rs) {
		...
		if (rs != null) {
			try {
				...
				MyLogAction.myCatch("/java", 214, ex);
			}
		}
	}
	...
	public static void close(PreparedStatement preparedStmt) {
		if (preparedStmt != null) {
			try {
				...
				MyLogAction.myCatch("/java", 225, ex);
			}
		}
	}
	...
	public int runScript(String[] sqlStatements) {
		for (int i = 0; i < sqlStatements.length; i++) {
			MyLogAction.log("running sql:" + sqlStatements[i]);
	...
	public static boolean listCount(String dbName) {
		...
		if (dbName.equalsIgnoreCase("CC")) {
			...
			MyLogAction.log("*** in runScript dbName is invalid and =" + dbName);
			System.exit(1);
		}
	...
	public static boolean listOracleDBTableNames(Connection conn) {
		...
		try {
			...
			while (rs.next()) {
				...
				MyLogAction.log("tableName=" + tableName);
			}
			...
			MyLogAction.myCatch("DBDataStandAlone", 389, ex);
		}
		return false;
	}
	...
	public static void listInitialRows(Connection con, String query) {
		...
		ResultSet rs = null;
		MyLogAction.myIO("listing initialRows for " + query);
		MyLogAction.log("listing initialRows for " + query);
		try {
			Statement s = con.createStatement();
			// MyLog.log("Reading rows from " + tokens[0] + " table");
			rs = s.executeQuery(query);
			MyLogAction.myIO("got result set");
			//
			ResultSetMetaData rsmd = rs.getMetaData();
			...
			MyLogAction.log("Total # of rows:" + i);
			MyLogAction.myIO("Total # of rows:" + i);
		} catch (Exception e) {
	...
	public static boolean listCountOfRowsInAllTables(String dbName,
		...
		try {
			...
			try {
				...
				MyLogAction.myCatch("/java", 361, e);
				e.printStackTrace();
			}
			try {
				rs = dmd.getTables(null, null, null, null);
			} catch (SQLException e) {
				MyLogAction.myCatch("/java", 367, e);
				e.printStackTrace();
			}
			int rows = 0;
			int i = 0;
			...
			while (rs.next()) {
				...
				try {
					...
					if (rs2.next()) {
						rows = rs2.getInt(1);
						MyLogAction.log("rows in table " + tablename + " = " + rows);
						MyLogAction.myReport(tablename + "\t\t\t\t " + rows);
					}
				} catch (SQLException se) {
					MyLogAction.myCatch("/java", 387, se);
					rows = 0;
				} catch (NullPointerException npe) {
					MyLogAction.myCatch("/java", 390, npe);
					rows = 0;
				} finally {
					MyLogAction.log("in finally of listcount");
					close(rs2);
				}
			}

			MyLogAction.myReport("================== e n d   o f   r e p o r t ==="
					+ "===============\r\n");
			// MyLog.resetReportName();
		} catch (SQLException ex) {
			MyLogAction.myCatch("DBDataStandAlone", 383, ex);
		}
		close(rs);
		return true;
	}
	...
	public static boolean countAllRows(String dbName) {
		...
		try {
			for (i = 0; i < tablename.length; i++) {
				...
				if (rs2.next()) {
					rows = rs2.getInt(1);
					MyLogAction.log("rows in table " + tablename[i] + " = " + rows);
					MyLogAction.myReport(tablename[i] + "\t\t\t\t " + rows);
				}
			}
		} catch (SQLException ex) {
			MyLogAction.myCatch("/java", 447, ex);
			rows = 0;
		} catch (NullPointerException npe) {
			MyLogAction.myCatch("/java", 455, npe);
			rows = 0;
		} finally {
			MyLogAction.log("in finally of countallrows");
			close(rs2);
		}
	...
	public boolean dump(Connection[] connections, String reportName) {
		boolean result = true;
		AU1Action.w("in dump");
		AU1Action.reportName = reportName; // "/SQLScript.txt";

		AU1Action.writeText("", true); // start a anew file
	...
	public boolean dump(String reportName) {
		...
		try {
			...
			while (rs.next()) {
				...
				AU1Action.w(createSQL);
				String insertScript = generateInsertIntoTableScript(tablename);
				AU1Action.w(insertScript);
			}

			// rs = s.executeQuery(query);
		} catch (SQLException se) {
			MyLogAction.myCatch("/java", 483, se);
		}
		AU1Action.reportName = "/log.txt";
		return result;
	}

	boolean runScriptFromFile(String dbName, String fileName) {
		MyLogAction.log("in runScriptFromFile with " + dbName + ", " + fileName);
		System.out.println("in runScriptFromFile with " + dbName + ", "
				+ fileName);
		String[] lines = AU1Action.textFile2SQLLines(fileName);
	...
	public ResultSetMetaData getRSMetaData(String tablename) {
		...
		try {
			...
			MyLogAction.myCatch("/java", 522, se);
		}
		return result;
	}
	...
	public ResultSet runQuery(String query) {
		...
		try {
			...
			MyLogAction.myCatch("/java", 482, e);
			AU1Action.w("SQLException in SetUpDB.runQuery for query:\n" + query);
			e.printStackTrace();
		}
		return rs;
	}
	...
	public String generateInsertIntoTableScript(String tablename) {
		...
		MyLogAction.log("read from table: " + tablename);
		try {
			int i = 0;
			while ((rs.next()) && (i < 100)) {
				i++;
				MyLogAction.log("row #" + i);
			...
			MyLogAction.myCatch("/java", 594, se);
		}
		// AU1.w(insertScript);
		return insertScript;
	}
	...
	public void displayRows(String tablename) {
		...
		try {
			...
			MyLogAction.myCatch("/java", 590, ex);
			ex.printStackTrace();
			AU1Action.w("exception in SetUpDB.generateCreateTableScript");
		}
		return;
	}
	...
	public static void displayResultSet(ResultSet resultSet) {
		...
		try {
			...
			for (int j = 1; j <= columnCount; j++) {
				...
				MyLogAction.log("col name:" + columnName + ": columnValue:"
						+ columnValue + ":");
			}
		} catch (SQLException ex) {
			MyLogAction.log("exception in displayResultSet()");
			ex.printStackTrace();
		}
	}
	...
	java.sql.DatabaseMetaData getDBMetaData() {
		...
		try {
			...
			MyLogAction.myCatch("/java", 603, e);
			e.printStackTrace();
		}
		return dmd;
	}
	...
	public ResultSet getTables() {
		...
		try {
			...
			MyLogAction.myCatch("/java", 614, e);
			e.printStackTrace();
		}
		return rs;
	}
	...
	public String getDBModelInsertCode(String tablename, String modelName) {
		...
		try {
			...
			MyLogAction.log("Number Of Column is:" + numOfColumn);
	...
	public String getsetClassValuesFromResultSet(ResultSetMetaData rsmd) {
		...
		try {
			...
			MyLogAction.myCatch("/java", 658, ex);
		}
	...
	public String getToString(ResultSetMetaData rsmd) {
		...
		try {
			...
			MyLogAction.myCatch("/java", 656, ex);
		}
		return text;
	}
	...
	public String getInsertUpdate(ResultSetMetaData rsmd, String tablename) {
		...
		try {
			...
			MyLogAction.myCatch("/java", 721, ex);
		}
		return text;
	}
	...
	public String getGetInsertSql(ResultSetMetaData rsmd, String tablename) {
		...
		try {
			...
			MyLogAction.myCatch("/java", 666, ex);
		}
		return text;
	}
	...
	public String getSetterGetters(ResultSetMetaData rsmd) {
		...
		try {
			...
			MyLogAction.myCatch("DBDataStandAlone", 687, ex);
		}
		return text;
	}
	...
	String getJavaType(String rsmdColumnType) {
		...
		MyLogAction.log("Java type for " + rsmdColumnType + " is " + dataType);
		return dataType;
	...
	public String getInstances(ResultSetMetaData rsmd) {
		...
		try {
			...
			MyLogAction.myCatch("DBDataStandAlone", 652, ex);
		}
		return text;
	}
	...
	static String row2text(ResultSet rs, boolean printColumnNames) {
		...
		try {
			...
			MyLogAction.myCatch("DBDataStandAlone", 968, ex);
